...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        length = min(len(sentence), max_length)
        words = sentence[:length] + [zero_vector] * (max_length - length)
        labels = labels[:length] + [zero_label] * (max_length - length)
        masks = [True] * length + [False] * (max_length - length)
        ret.append((words, labels, masks))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embed_word = tf.Variable(self.pretrained_embeddings)
        _embeddings = tf.nn.embedding_lookup(embed_word, self.input_placeholder)
        embeddings = tf.reshape(_embeddings, shape=(-1, self.max_length,
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        _label = tf.boolean_mask(self.labels_placeholder, self.mask_placeholder)
        _preds = tf.boolean_mask(preds, self.mask_placeholder)
        ce = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=_label, logits=_preds)
